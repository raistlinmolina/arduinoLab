/*
 * f(Roll, Pitch) should give us an idea about how stable the shot is
 * We need to measure how much the puck rotates, Yaw rate of variation should give us this data.
 *
 * Acceleration of the puck should be given by f(accelX,accelY,accelZ,Rotation)
 *
 */

// Sketch based on the one from Jeff Rowberg.
...
#define REMOTEXY_WIFI_SSID "GTP"
#define REMOTEXY_WIFI_PASSWORD "ManuelMolina"
...
void initialize(){
	...
	if (devStatus == 0) {
		...
		attachInterrupt(D6, dmpDataReady, RISING);
...
void readableWorldAccel(){
    ...
    RemoteXY.shot1Level = abs(aaWorld.x)/100;
    RemoteXY.shot2Level = abs(aaWorld.y)/100;
    RemoteXY.shot3Level = abs(aaWorld.z)/100;
